...
The configuration depends on factors such as action requirements, data sensitivity, system complexity, regulatory constraints, expected reliability, available integration tools, information volume, decision consequences, and intermediate results.
...
The configuration is therefore dynamic rather than permanent. It may change when the action changes, new compliance regulations are introduced, an agentic result fails to satisfy expected metadata accuracy, or human interpretation identifies a new user adoption barrier.

Throughout this process, the document management specialist remains the professional responsible for directing and controlling the work associated with the occupation and its responsibilities. Agentic AI provides powerful complementary capabilities that expand what the professional can extract, classify, migrate, audit, automate, and document.
...
Evaluation criteria depend on the work being performed. In document management, they include metadata accuracy, workflow execution speed, system uptime, access control integrity, migration completeness, user adoption rates, and search retrieval relevance.
...
For example, an AI agent is configured to automatically classify incoming invoices and extract vendor metadata using NLP. However, task performance evaluation reveals that the agent frequently misclassifies documents from international vendors due to varying currency symbols and date formats. The agentic skill is then refined through the integration of localized formatting rules, expanded training datasets, or stricter confidence-score thresholds that route ambiguous documents to a human queue. The revised skill is evaluated again, and once it satisfies the required performance criteria, it is retained. Human skills follow a corresponding development process: when a specialist is tasked with migrating the organization to a new cloud-based DMS architecture, the relevant capability is developed through vendor certification, sandbox experimentation, and peer collaboration before its performance is evaluated through the successful execution of the migration project.
...
The skill space is not constructed as an isolated collection of occupation-specific representations. Many human and agentic skills used in document management are reusable across occupations. Taxonomy design, workflow automation, access control auditing, data migration, system integration, and user training contribute to work in IT systems administration, data governance, knowledge management, business analysis, and many other fields.
...
This cycle allows human and agentic capabilities to evolve alongside the requirements of document management work.
...
### 1.4.2. Information Architecture and Metadata Taxonomy Design

Information architecture and metadata taxonomy design structures how digital content is organized, tagged, and retrieved across the enterprise. This responsibility involves defining folder hierarchies, creating standardized metadata fields, establishing controlled vocabularies, and ensuring that the system's architecture aligns with how users naturally search for and interact with information.

Agentic skills are well suited to analyzing thousands of legacy filenames to suggest common keywords, applying bulk metadata tags to existing repositories, and identifying orphaned files that lack required classification data. Human skills contribute to understanding the distinct retrieval needs of different business units, designing logical and intuitive taxonomies that prevent user frustration, and resolving conflicts when different departments demand contradictory metadata structures.

For example, an organization struggles to retrieve historical project proposals because users save files with inconsistent naming conventions on local drives. The specialist defines a mandatory metadata schema requiring fields for project code, client name, and document stage. An AI agent then scans the legacy file shares, uses NLP to extract proposed client names and project codes from the document text, and applies these as draft metadata tags during the ingestion process. The specialist reviews a sample of the agent's classifications, identifies that the agent is confusing internal project codenames with external client names, and refines the classification rules before authorizing the bulk migration.

| Human skills|Agentic skills|Integrated collaboration|
| ---|---|---|
| taxonomy design, user behavior analysis, conflict resolution, information governance|keyword extraction, bulk metadata application, orphan file detection, NLP classification|The AI agent scales the extraction and application of metadata across vast repositories, while the specialist designs the logical taxonomy and ensures the classification rules reflect true business contexts.|

### 1.4.3. Workflow Automation and Process Optimization

Workflow automation and process optimization transforms manual, email-based document routing into structured, system-driven lifecycles. This responsibility includes mapping business processes, configuring automated approval routes, setting up digital signature integrations, and designing notification triggers to ensure documents move efficiently through their required review stages.

Agentic skills assist with workflow scripting, automated notification routing, bottleneck detection via system logs, and the generation of process mapping diagrams. Human skills are especially important for interviewing stakeholders to understand the actual business rules governing approvals, identifying unnecessary bureaucratic steps that slow down operations, and designing fallback procedures for when a designated approver is unavailable.

For example, the legal department requests an automated workflow for non-disclosure agreements (NDAs). The specialist interviews the legal team to map the exact conditions under which an NDA requires executive versus managerial approval. An AI agent then generates the workflow configuration scripts, integrates the digital signature API, and sets up the automated email reminders for pending signatures. The specialist tests the workflow and realizes that the system lacks a mechanism to route the document back to the originator if the counterparty requests redlines. The specialist revises the process logic to include a "return for revision" state, and the agent updates the workflow configuration accordingly.

| Human skills|Agentic skills|Integrated collaboration|
| ---|---|---|
| process mapping, stakeholder interviewing, bottleneck resolution, fallback design|workflow scripting, API integration, notification routing, log analysis|The AI agent builds and executes the technical routing of the workflow, while the specialist defines the business logic and optimizes the process for real-world operational friction.|

### 1.4.4. Security, Access Control, and Information Governance Enforcement

Security, access control, and information governance enforcement ensures that sensitive digital content is only accessible to authorized personnel and that technical configurations align with organizational data policies. This responsibility includes managing workspace permissions, configuring ethical walls or security groups, auditing access logs for anomalous behavior, and technically enforcing document retention and disposition schedules defined by governance teams.

Agentic skills contribute to continuous permission auditing, anomaly detection in access logs, automated execution of retention policies (e.g., moving expired documents to cold storage), and scanning for sensitive data patterns (like PII or PHI) within unprotected workspaces. Human skills define the security policies, interpret the business context of unusual access patterns to determine if a breach has occurred, and authorize the permanent deletion of records that have met their legal retention requirements.

For example, an automated governance scan alerts the specialist that a highly confidential M&A workspace has been accessed by a user outside the designated deal team. The AI agent parses the access logs, traces the user's session, and identifies that the user accessed the workspace via a shared, poorly configured secondary link. The specialist reviews the evidence, confirms this was an accidental policy violation rather than malicious exfiltration, and revokes the user's access. The specialist then instructs the agent to run a comprehensive audit of all secondary links in confidential workspaces and drafts a targeted security reminder for the deal team leads.

| Human skills|Agentic skills|Integrated collaboration|
| ---|---|---|
| security policy definition, breach investigation, risk authorization, governance enforcement|permission auditing, anomaly detection, retention execution, sensitive data scanning|The AI agent continuously monitors access patterns and enforces automated retention rules, while the specialist investigates security anomalies and authorizes high-risk governance actions.|

### 1.4.5. System Integration, Migration, and Data Quality Management

System integration, migration, and data quality management connects the DMS with other enterprise applications (like CRM or ERP systems) and ensures the integrity of data moving into or out of the repository. This responsibility includes designing ETL (extract, transform, load) pipelines, mapping fields between disparate systems, executing bulk migrations, and implementing deduplication protocols to maintain a single source of truth.

Agentic skills retrieve API documentation, write data transformation scripts, execute deduplication algorithms, perform OCR on scanned legacy documents, and monitor integration error logs. Human skills evaluate the logical mapping between different systems' data models, determine which duplicate record should be preserved as the master copy, and manage the operational risks associated with migrating mission-critical data.

For example, the organization acquires a smaller company and must merge the acquired company's document repository into the enterprise DMS. An AI agent analyzes both databases, identifies thousands of duplicate policy documents based on content hashing, and drafts the SQL scripts required to map the acquired company's metadata fields to the enterprise standard. The specialist reviews the mapping logic and identifies that the acquired company uses a fundamentally different fiscal year calendar that will break the enterprise's automated retention calculations if merged directly. The specialist designs a data transformation rule to normalize the dates, instructs the agent to update the migration scripts, and oversees the final data validation.

| Human skills|Agentic skills|Integrated collaboration|
| ---|---|---|
| data mapping logic, master data judgment, migration risk management, system integration design|ETL scripting, deduplication execution, OCR processing, error log monitoring|The AI agent accelerates the technical execution of data transformation and deduplication, while the specialist governs the logical mapping and ensures data integrity across complex system boundaries.|
